...
- [x] Repair the restored Auth machine credentials through the private
  Infisical UI, verify an authenticated read of `AUTH_RESEND_API_KEY` at
  `/niu-auth` in `prod`, and switch the deployed Auth read path to
