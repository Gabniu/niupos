...
function SignInForm() {
  ...
  return <AuthFrame eyebrow="Secure access" title="Sign in" description="Sign in to manage your workspace, team, and connected applications." footer={<>Need access? <Link className="font-medium text-blue-600" href="/sign-up">Create an account</Link></>}>
    <form className="space-y-6" onSubmit={submit}>
      {setupComplete && <p className="rounded-lg border border-blue-200 bg-blue-50 p-3 text-xs leading-5 text-blue-800" role="status">Administrator setup is complete. Sign in with the account you just created.</p>}
      ...
      <div className="flex items-center justify-between gap-4 text-sm"><label className="flex items-center gap-3 text-slate-500"><input className="size-[18px]" name="remember" type="checkbox" />Remember me</label><Link className="font-medium text-blue-600" href="/forgot-password">Forgot password?</Link></div>
      {error && <p className="rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-700" role="alert">{error}</p>}
      <button className={primaryButton} disabled={pending} type="submit">{pending ? "Signing in..." : "Sign in to workspace"}</button>
...
export default function SignInPage() {
  return <Suspense fallback={<AuthFrame eyebrow="Secure access" title="Sign in" description="Sign in to manage your workspace, team, and connected applications." footer={<>Loading sign in...</>}><div className="h-48" /></AuthFrame>}><SignInForm /></Suspense>;
}
